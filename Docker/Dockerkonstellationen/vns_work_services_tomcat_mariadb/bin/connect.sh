...
ssh work cat .ssh/id_rsa.pub | docker exec -i --user user mariadb  tee -a /home/user/.ssh/authorized_keys &>/dev/null
ssh work 'ssh-keygen -q -R mariadb; ssh-keyscan mariadb>> .ssh/known_hosts' &>/dev/null
